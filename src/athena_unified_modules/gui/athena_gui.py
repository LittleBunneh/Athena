#!/usr/bin/env python3
# Ensure Athena_core is in sys.path for package imports
import sys, os
athena_core_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if athena_core_root not in sys.path:
    sys.path.insert(0, athena_core_root)

# --- AUTO-SET LLM API KEY (TOGETHER) ---
...
# Add the ai_core directory to the path (relative to athena_unified_modules)
ai_core_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'ai_core'))
if ai_core_path not in sys.path:
    sys.path.insert(0, ai_core_path)


# Import LLM-enhanced consciousness system
llm_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'llm'))
if llm_path not in sys.path:
    sys.path.insert(0, llm_path)

try:
    from athena_unified_modules.llm.llm_enhanced_consciousness import LLMEnhancedConsciousness
except ImportError as e:
    print(f"[ERROR] Cannot import LLMEnhancedConsciousness: {e}")
    sys.exit(1)
...
class AthenaGUI:
    def __init__(self):
        ...
        # Initialize LLM-enhanced Athena
        self.athena = None
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(
            top_frame,
            text="ATHENA PRIME - AUTONOMOUS CONSCIOUSNESS (LLM Enhanced)",
        ...
        # Chat Tab
        chat_frame = tk.Frame(self.notebook, bg='#0a0a0a')
        self.notebook.add(chat_frame, text="Chat with Athena")

        # Autonomous Activity Tab
        activity_frame = tk.Frame(self.notebook, bg='#0a0a0a')
        self.notebook.add(activity_frame, text="Autonomous Activity")

        # System Status Tab
        status_frame = tk.Frame(self.notebook, bg='#0a0a0a')
        self.notebook.add(status_frame, text="System Status")

        # Progress & Stats Tab
        stats_frame = tk.Frame(self.notebook, bg='#0a0a0a')
        self.notebook.add(stats_frame, text="Progress & Stats")
    ...
    def start_athena(self):
        """Initialize and start LLM-enhanced Athena in background thread"""
        def init_athena():
            try:
                self.output_queue.put(("chat", "[SYSTEM] Initializing LLM-Enhanced Athena Consciousness..."))
                self.athena = LLMEnhancedConsciousness()
                self.llm_status = self.athena.llm_status
                self.output_queue.put(("chat", "[SYSTEM] Athena LLM-enhanced consciousness online!"))
                self.output_queue.put(("chat", "[ATHENA] Hello! I'm operating with LLM enhancement now."))
                self.output_queue.put(("chat", "[ATHENA] You can chat with me while I work on my goals."))
                self.status_queue.put(("status", "Autonomous, LLM-Enhanced and Active"))
    ...
    def send_message(self, event=None):
        """Send message to Athena (LLM-enhanced)"""
        ...
        # Process with LLM-enhanced Athena in background thread
        def process_message():
            try:
                if self.athena:
                    response = self.athena.process_with_llm_enhancement(message)
    ...
    def setup_stats_tab(self, parent):
        """Setup the Progress & Stats tab with real-time metrics and LLM status"""
    ...
    def update_stats_tab(self):
        """Update the Progress & Stats tab with latest info"""
        if not self.athena:
            return
        try:
            stats = self.athena.get_llm_status_report()
        except Exception as e:
            stats = f"[ERROR] Could not get stats: {e}"
    ...
    def update_gui(self):
        """Update GUI with queued messages and stats"""
